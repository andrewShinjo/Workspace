...
3. **Generalize the input space.** For each conditional, enumerate the full set of possible input values or states. Consider orthogonal variables that aren't checked by any conditional but should be (e.g., selection length when only location is checked, or empty state when only existence is checked).
